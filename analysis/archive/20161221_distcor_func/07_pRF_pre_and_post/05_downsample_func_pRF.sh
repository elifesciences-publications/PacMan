...
strPathTarget="/media/sf_D_DRIVE/MRI_Data_PhD/05_PacMan/20161221/nii_distcor/func_regAcrssRuns_cube_up/down/"
...
varUpInv=`bc -l <<< 1.0/${varUpFac}`

# Save original path in order to cd back to this path in the end:
strPathOrig=( $(pwd) )

# cd into target directory and create list of images to be processed:
cd "${strPathTarget}"
aryIn=( $(ls) )
