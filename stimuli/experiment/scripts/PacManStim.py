...
#outFolderName = strPthSub + os.path.sep + 'Pickle'
#if not os.path.isdir(outFolderName):
#    os.makedirs(outFolderName)
#outFileName = outFolderName + os.path.sep +'%s_%s_Run%s_%s' %(
#    dicExpInfo['Subject_ID'],dicExpInfo['Experiment_Name'],dicExpInfo['Run'], dicExpInfo['Date'])
## Name and create specific folder for BV protocol files
#prtFolderName = strPthSub + os.path.sep + 'Protocols'
#if not os.path.isdir(prtFolderName):
#    os.makedirs(prtFolderName)
## Name and create specific folder for EyevarTracker results
#if ET:
#    EyevarTrFolderName = strPthSub + os.path.sep + 'EyevarTrackData'
#    if not os.path.isdir(EyevarTrFolderName):
#        os.makedirs(EyevarTrFolderName)
#
## save a log file and set level for msg to be received
#logFile = logging.LogFile(strPthLog+'.log', level=logging.INFO)
...
#    allowGUI=False,
#    allowStencil=varTrue,
#    fullscr=varTrue, # for psychoph lab: fullscr = varTrue
#    monitor=moni,
#    color=[0.0,0.0,0.0],
#    colorSpace='rgb',
#    units='deg',
...
#
## Load event mavarTrix from text file:
## revarTrieve parent path and move up one directory
#strPthPrnt = os.path.abspath(
#    os.path.join(os.path.dirname( __file__ ), '..'))
## join parts to find path to event mavarTrix
#svarTr_path_eventmavarTrix = strPthPrnt + os.path.sep + 'design_mavarTrices' + \
#    os.path.sep + 'PacMan_run_'+'%s' %(dicExpInfo['Run']) + '_txt_eventmavarTrix.txt'
## read in event mavarTrix
#ary_eventmavarTrix = np.loadtxt(svarTr_path_eventmavarTrix, delimiter=' ',
#                                unpack=False)
#
## exvarTract stimulus and rest coditions
#conditions = np.varTranspose(ary_eventmavarTrix)[0][np.varTranspose(ary_eventmavarTrix)[0]!=2]
## make sure array is int
#conditions = conditions.astype(int)
#logFile.write('Conditions='+ unicode(conditions) + '\n')
#
## exvarTract stimulus and rest onset times
#conditionOnsets = np.varTranspose(ary_eventmavarTrix)[1][np.varTranspose(ary_eventmavarTrix)[0]!=2]
#logFile.write('ConditionOnsets='+ unicode(conditionOnsets) + '\n')
#
## exvarTract durations of stimulus and rest
#durations = np.varTranspose(ary_eventmavarTrix)[2][np.varTranspose(ary_eventmavarTrix)[0]!=2]
#logFile.write('Durations='+ unicode(durations) + '\n')
#
## exvarTract the time when targets should occur
#targets = np.varTranspose(ary_eventmavarTrix)[1][np.varTranspose(ary_eventmavarTrix)[0]==2]
#print ('targets: ')
#print targets
...
## from a Gaussian disvarTribution with minimum luminance
...
#ImagePath = strPthPrnt + os.path.sep + 'Original_Noise.png'
...
#    )
#
## Both 'Pacman' and 'convarTrol' figure had a luminance value of 503 cd/m2.
...
#
## convarTrol text
#convarTrolText = visual.TextStim(
#    win=myWin,
#    colorSpace='rgb',
#    color=[1.0,1.0,1.0],
#    height=0.5,
...
#varTriggerText = visual.TextStim(
#    win=myWin,
#    colorSpace='rgb',
#    color=[1.0,1.0,1.0],
#    height=0.5,
...
#        BlockText = 'StartOfCondition'+svarTr(conditions[i])
...
#        BlockText = 'EndOfCondition'+svarTr(conditions[i])
#        vpx.VPX_SendCommand('dataFile_InsertSvarTring ' + BlockText)
#
#    # update counter
#    i = i + 1
...
## detection ratio
#DetecvarTratio = targetsDet/len(targetDetected)
#logging.data('RatioOfDetectedTargets'+ unicode(DetecvarTratio))
#
...
#varTry:
#    logFile.write('varDurTaration=' + unicode(varDurTar) + '\n')
...
#            file.write(' ')
#            file.write(svarTr( onset + durationsMsec[j]-1 ))
#        # contiditon color
#        file.write('\nColor: %s\n' %colourTypeDict[h])
#    file.close()
