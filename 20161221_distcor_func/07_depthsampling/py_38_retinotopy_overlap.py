...
strNiiX = '/media/sf_D_DRIVE/MRI_Data_PhD/04_ParCon/20161219_02/nii_distcor/retinotopy/pRF_results_up/pRF_results_x_pos_up.nii.gz'  #noqa

# Path of nii file with y-coordinates of pRFs:
strNiiY = '/media/sf_D_DRIVE/MRI_Data_PhD/04_ParCon/20161219_02/nii_distcor/retinotopy/pRF_results_up/pRF_results_y_pos_up.nii.gz'  #noqa

# Path of nii file with pRF size estimates (SD):
strNiiSd = '/media/sf_D_DRIVE/MRI_Data_PhD/04_ParCon/20161219_02/nii_distcor/retinotopy/pRF_results_up/pRF_results_SD_up.nii.gz'  #noqa

# Path of nii file with R2 values:
strNiiR2 = '/media/sf_D_DRIVE/MRI_Data_PhD/04_ParCon/20161219_02/nii_distcor/retinotopy/pRF_results_up/pRF_results_R2_up.nii.gz'  #noqa

# Output file base name:
strNiiOt = '/media/sf_D_DRIVE/MRI_Data_PhD/04_ParCon/20161219_02/nii_distcor/retinotopy/pRF_results_up/pRF_results_'  #noqa
...
# Define the inner and outer radius of the annulus stimulus for which the pRF
# overlap will be calculated - in degree of visual angle:
varStimRadOut = 4.0
varStimRadIn = 1.5
...
# Matrix representing the annulus (i.e. the visual stimulus):
aryLgcStim = (aryDist <= varStimRadOut) * (aryDist >= varStimRadIn)

# Create matrix that will contain the overlap ratio for each voxel:
aryRatio = np.zeros(aryNiiR2.shape, dtype=np.float32)

# Create matrix that will contain the binary map for pRF centre overlap with
# stimulus (i.e. whether the pRF centre is on the stimulus)
aryCentre = np.zeros(aryNiiR2.shape, dtype=np.float32)
...
# Logical test for voxel inclusion: is the R2 value larger than the specified
# threshold:
...
# Vector with the indicies at which the nii data will be separated in order to
# be chunked up for the parallel processes:
...
# Create list for vectors with results, in order to put the results into the
...
# Arrays for stimulus-pRF overlap results (which will be used to get data into
# the original shape):
...
# stimulus. We first create a cell array that will contain the matrices with
# the masks for each threshold value (several mask with different thresholds
...
# Save nii to disk:
nib.save(niiOtRatio, (strNiiOt + 'ovrlpRatio.nii'))
nib.save(niiOtCentre, (strNiiOt + 'ovrlpCentre.nii'))
...
for idxMsk in range(0, varNumMsk):
    ...
    # Save nii to disk:
    strTmp = (strNiiOt + 'ovrlpMask_' + str(lstOvrlp[idxMsk]) + 'prct.nii')
